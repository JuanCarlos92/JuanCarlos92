...
Antes de empezar, hay que detallar algunas variables que se usarán en el siguiente ejemplo:
...
La sintaxis de un disparador en Oracle es la siguiente:

```plsql
CREATE OR REPLACE TRIGGER <name>
<time> <evento> ON <nombre_tabla>
FOR EACH ROW
BEGIN
<cuerpo>
END;
...
```plsql
CREATE OR REPLACE TRIGGER tr_insert_persona
...
El cuerpo del disparador es el código que se ejecuta cuando se lanza el disparador. Dentro de un disparador nos podemos referir a los datos que se están insertando, modificando o borrando usando `:NEW` y `:OLD`. Esto nos permite evitar que se borren o modifiquen datos indeseados; aplicar condiciones como por ejemplo, que haya stock suficiente para un producto, etc.

Por ejemplo, controlar la edad de una persona para evitar que se inserten menores de edad:

```plsql
CREATE OR REPLACE TRIGGER tr_update_persona
...
BEGIN
	SELECT nombre INTO nombre_mascota FROM mascotas WHERE id = :NEW.id;
